...
export default function HomeLoanCalculator() {
  ...
  const { updateURLParams, clearURLParams } = useURLParams({
    ...
    storeSetters: {
      ...
      monthlyServiceFee: setMonthlyServiceFee,
  ...
  const handleCalculate = () => {
    ...
    updateURLParams({
      ...
      loanTerm,
      termUnit,
      monthlyServiceFee: effectiveServiceFee,
      isAdvancedMode,
    });
  ...
  useEffect(() => {
    ...
    // Check immediately if we have URL params and show loader
    ...
    // Wait a bit for URL params to be loaded into store
    const timer = setTimeout(() => {
      // Determine if URL contains any relevant params (re-check in case URL changed)
      const stillHasRelevantParams = [
        'price',
        'deposit',
        'rate',
        'term',
      ...
      // Only validate if URL actually had our params
      if (stillHasRelevantParams) {
        // Read raw URL params (avoid race with store sync)
        const rawPrice = searchParams.get('price');
        const rawDeposit = searchParams.get('deposit');
        const rawRate = searchParams.get('rate');
        const rawTerm = searchParams.get('term');
        const rawTermUnit = searchParams.get('termUnit');
        ...
        const priceValue = parseNum(rawPrice);
        const depositValue = parseNum(rawDeposit) ?? 0;
        const rateValue = parseNum(rawRate);
        const termValue = parseNum(rawTerm);
        const termUnitParam =
          rawTermUnit === 'months' || rawTermUnit === 'years'
            ? rawTermUnit
            : 'years';
        const termInYears =
          termValue && termValue > 0
            ? termUnitParam === 'months'
              ? termValue / 12
              : termValue
            : 0;
        ...
        if (!isValid) {
          // Invalid URL params: clear and reset form
          clearURLParams();
          resetForm();
        ...
        // Sync store from URL explicitly to avoid race with URL-sync effect
        const decode = (v: string | null) =>
          v === null ? '' : decodeURIComponent(v);
        setPropertyPrice(decode(rawPrice));
        setDeposit(decode(rawDeposit));
        setInterestRate(decode(rawRate));
        setLoanTerm(decode(rawTerm));
        setTermUnit(termUnitParam as TermUnit);

        // Optional params
        const rawServiceFee = searchParams.get('serviceFee');
        if (rawServiceFee !== null) {
          setMonthlyServiceFee(decode(rawServiceFee));
        }
        const rawAdvanced = searchParams.get('advanced');
        if (rawAdvanced !== null) {
          setIsAdvancedMode(rawAdvanced === '1' || rawAdvanced === 'true');
        }

        // Perform calculation directly from the validated URL params to avoid races
        const loanAmountFromURL =
          (priceValue as number) - (depositValue as number);
        const calculatedResultsFromURL = calculateHomeLoan(
          loanAmountFromURL,
          rateValue as number,
          termInYears
        );

        // Determine effective service fee based on advanced flag from URL
        const serviceFeeRaw = searchParams.get('serviceFee');
        const advancedRaw = searchParams.get('advanced');
        const isAdvancedFromURL = advancedRaw === '1' || advancedRaw === 'true';
        const effectiveServiceFeeFromURL = isAdvancedFromURL
          ? decode(serviceFeeRaw)
          : '69';

        const serviceFeeValueFromURL = parseFloat(
          effectiveServiceFeeFromURL || '0'
        );

        setResults({
          ...calculatedResultsFromURL,
          serviceFee: isNaN(serviceFeeValueFromURL)
            ? 0
            : serviceFeeValueFromURL,
        });
        setIsDirty(false);
      ...
      // No relevant URL params - mark initialized
      hasInitializedFromURL.current = true;
      setIsLoadingFromURL(false);
    }, 200); // Give time for URL params to load into store

    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // Only run once on mount - URL params are loaded via useURLParams hook
    ...
    marginToPrime !== null
      ? marginToPrime === 0
        ...
        : marginToPrime > 0
          ...
          : `Prime - ${Math.abs(marginToPrime).toFixed(2)}%`
      : null;
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <button
                type="button"
                onClick={() => {
                  // Clear URL first to avoid the URL-sync effect repopulating fields
                  clearURLParams();
                  // Then reset store values
                  resetForm();
                }}
                  ...
                  <div className="mt-8 border-t border-gray-200 pt-8">
                    ...
                    <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-4">
                      <p className="mb-2 text-sm text-gray-700">
                        <strong>💡 How this works:</strong> Historical
                        comparisons show what <em>your</em> rate would have been
                        based on your margin to prime. If you&apos;re getting
                        prime minus 1%, these calculations show historical prime
                        rates minus 1%.
                      </p>
                      <p className="text-sm text-gray-700">
                        <strong>Tip:</strong> Even a 0.25% rate change can
                        significantly impact your monthly budget. It&apos;s
                        worth shopping around and negotiating with different
                        banks.{' '}
